...
package com.glsebastiany.smartcatalogspl.core.presentation.persistence.datafetcher;
...
import com.glsebastiany.smartcatalogspl.core.presentation.persistence.firebase.FirebaseItem;
...
public class ItemsBasicUpdater implements FirebaseUpdatable {
    private static final String LOG_TAG = ItemsBasicUpdater.class.getSimpleName();

    private final SharedPreferencesUpdate_ sharedPreferencesUpdate;
    private final ItemBasicUseCases itemUseCases;


    @Inject
    public ItemsBasicUpdater(Context context, ItemBasicUseCases itemBasicUseCases) {
        this.sharedPreferencesUpdate = new SharedPreferencesUpdate_(context);
        this.itemUseCases = itemBasicUseCases;
    }
    ...
    public Set<String> getLocalIds(int expectedSize) {
        ...
        List<ItemBasicModel> allItems = itemUseCases.getAll().toList().toBlocking().single();
    ...
    public long insert(DataSnapshot snapshot) {
        ...
        itemUseCases.remove(String.valueOf(firebaseItem.getId()));
        itemUseCases.insert(firebaseItem.toBasicItemModel());
